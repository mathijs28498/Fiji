...
use crate::rendering::{
    ...
    render_objects::shared::{BufferContainer2D, Vertex2D, BufferContainer2DUv},
};

pub(crate) mod figure_vs {
    vulkano_shaders::shader! {
        ty: "vertex",
        path: "src/shaders/shaders_2d/figure_pipeline.vert",
        types_meta: {
...
pub(crate) mod figure_fs {
    vulkano_shaders::shader! {
        ty: "fragment",
        path: "src/shaders/shaders_2d/figure_pipeline.frag",
        types_meta: {
            use bytemuck::{Pod, Zeroable};
...
pub(crate) struct FigurePipeline {
    vs: Arc<ShaderModule>,
    fs: Arc<ShaderModule>,
...
impl FigurePipeline {
    pub(crate) fn new(device_container: &DeviceContainer) -> Self {
        let vs = figure_vs::load(device_container.device().clone()).unwrap();
        let fs = figure_fs::load(device_container.device().clone()).unwrap();
    ...
    pub(crate) fn draw(
        ...
        push_constants: figure_fs::ty::Constants,
    ) {
